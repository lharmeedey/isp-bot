...
async function managePlans(ctx) {
  ...
  if (plansRes.rows.length) {
    currentPlans = plansRes.rows;
    source       = 'custom plans';
  } else {
    currentPlans = JSON.parse(process.env.PLANS || '[]');
    source       = 'global plans';
  }

  const lines = currentPlans.map(p =>
    `• ${p.label}: N${Number(p.price).toLocaleString('en-NG')} | ${p.gb}GB | ${p.validity}`
  ).join('\n');

  return ctx.reply(
`Plans for ${tenant.name} (${source})
...
Use underscore for spaces in validity e.g. 7_days or 30_days

Example:
/setplan ${tenantId} 5GB 1500 5 7_days 6a64e90e5c7bdd073c22b522
...
async function setPlan(ctx) {
  ...
  if (parts.length < 7) {
    return ctx.reply(
      ...
      'Use underscore for spaces in validity\n\n' +
  ...
  const validity       = parts[5].replace(/_/g, ' ');
  const omadaProfileId = parts[6];

  if (isNaN(price) || isNaN(gb)) {
    return ctx.reply('Invalid price or GB value. Both must be numbers.');
  }
...
Plan:    ${label}
Price:   N${price.toLocaleString('en-NG')}
Data:    ${gb}GB
Period:  ${validity}
Profile: ${omadaProfileId}

Run /reloadtenant ${tenantId} to apply changes immediately.`
  );
}
...
async function resetPlans(ctx) {
  ...
  logger.info('Tenant plans reset to global', { tenantId });

  return ctx.reply(
`Plans reset to global defaults for ${tenantRes.rows[0].name}
